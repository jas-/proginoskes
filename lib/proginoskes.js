...
var version = 'v0.1.9'
  ...
  var libs = libs || {
     ...
     * @parm {Boolean} digest Whether or not to provide a digest vs. a stream
     *
     * return {Array}
     */
    tail: function(logs, digest) {
      var ret = [];

      if (logs.length >= 1) {
        logs.forEach(function(log){
          (digest) ? ret.push('tail -n10 '+log) : ret.push('tail -f -n0 '+log);
        });
      }
    ...
    functions: function(obj) {
      ...
      if (obj.hosts.length >= 1) {
        obj.hosts.forEach(function(host) {
          ...
          var logs = libs.tail(host.logs, (host.digest || obj.global.digest));
          ...
          host.funcs.push(function fn(callback) {
            ...
            conn.on('error', function error(err) {
              ...
              logs.forEach(function tail(log) {
                host.conn.exec(log, function cb(err, stdout, stderr) {
