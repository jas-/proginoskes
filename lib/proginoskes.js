...
var version = 'v0.1.9'
  ...
  var config = config || {
        ...
        if (obj.hosts.length >= 1) {
          obj.hosts.forEach(function(value) {
            ...
            if (value.hasOwnProperty('debug') || obj.global.debug) {
              hosts.debug = ((hosts.loglevel > 0) ? console.log : true);
            }
            ...
            // use password assigned to host vs. global
            ...
            // use private key assigned to host vs. global
            ...
            // use algorithms object assigned to host vs. global
            if (value.hasOwnProperty('algorithms')) {
              hosts.algorithms = config.algorithms(value.algorithms,
                                                   obj.global.algorithms);
            } else {
              if (obj.global.hasOwnProperty('algorithms')) {
                hosts.algorithms = config.algorithms({}, obj.global.algorithms);
              }
            }
     ...
     * @function algorithms
     * @scope private
     * @abstract Setup host algorithms
     *
     * @param {Object} obj Configuration object
     * @param {Array} defined Defined logs array
     *
     * @returns {Array}
     */
    algorithms: function(obj, defined) {
      return libs.merge(obj, defined);
    }
  }
  ...
  var libs = libs || {
     ...
     *           to libs.stream()
     *
     * @param {Obect} obj object with connection param(s)
     *
     * @returns {Array}
     */
    functions: function(obj) {
      ...
      if (obj.hosts.length >= 1) {
        obj.hosts.forEach(function(host) {
          host.funcs = funcs;
          ...
          host.funcs.push(function fn(callback) {
            ...
            host.conn.on('error', function error(err) {
              callback({
            ...
            host.conn.on('ready', function ready() {
              ...
              logs.forEach(function(log) {
                ...
                host.conn.exec(log, function cb(err, stdout) {
    ...
    stream: function(stream, fn) {
      ...
      stream.stream.on('close', function() {
  ...
  var init = function(options, fn) {
    ...
    if (opts.hasOwnProperty('hosts') && opts.hosts.length >= 1) {
      funcs = libs.merge(funcs, libs.functions(opts));
    } else {
